                            ...
                            <div class="single-table">
                                <div class="table-responsive">
                                    ...
                                    <?php foreach($imoveis as $imovel):?>
                                        <tr>
                                            <th scope="row"><?=$imovelId()?></th>
                                            <td><?=$imovelIdentificacao()?></td>
                                            <td><?=$imovelMatricula()?></td>
                                            <td><?=$imovelInscricaoImobiliaria()?></td>
                                            <td><?=$imovelLogradouro()?></td>
                                            <td><?=$imovelNumeroLogradouro()?></td>
                                            <td><?=$imovelRua()?></td>
                                            <td><?=$imovelBairro()?></td>
                                            <td><?=$imovelCidade()?></td>
                                            <td><?=$imovelEstado()?></td>
                                            <td><?=$imovelCep()?></td>
                                            <td><?=$imovelIbge()?></td>
                                            <td><?=$imovelMetrosQuadrados()?></td>
                                            <td><?=$imovelQuartos()?></td>
                                            <td><?=$imovelBanheiros()?></td>
                                            <td><?=$imovelVagasGaragem()?></td>
                                            <td><?=$imovelAtivo()?></td>
                                            <td><?=$imovelCriado()->format('Y-m-d H:i:s')?></td>
                                            <td><?=$imovelModificado()->format('Y-m-d H:i:s')?></td>
                                            <td><?=$imovelCriadorId()?></td>
                                            <td><?=$imovelModificadorId()?></td>
                                            
                                            <td>
                                                <ul class="d-flex justify-content-center">
                                                    <li class="mr-3"><a href="/src/View/adminCrud/Imovel/update.php?id=<?=$imovelId()?>&identificacao=<?=$imovelIdentificacao()?>" class="btn btn-inverse-warning"><i class="bi bi-pencil-square mr-1"></i>Edit</a></li>
                                                    <form method="POST">
                                                        <input type="hidden" name="delete_id" value="<?=$imovelId()?>">
