...
class CaracteristicasController extends AppController
{
    ...
    {
        $caracteristica = $this->Cara->newEmptyEntity();
        if ($this->request->is('post')) {
            $caracteristica = $this->Cara->patchEntity($caracteristica, $this->request->getData());
            if ($this->Cara->save($caracteristica)) {
                $this->Flash->success(__('Imóvel salvo com sucesso.'));
                return $this->redirect(['action' => 'index']);
            }
            $this->Flash->error(__('Erro ao salvar o imóvel.'));
        }
        $this->set('caracterisc$caracteristica', $caracteristica);
    }
    }
