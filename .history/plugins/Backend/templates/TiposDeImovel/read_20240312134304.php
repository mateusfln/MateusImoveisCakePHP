                            ...
                            <div class="single-table">
                                <div class="table-responsive">
                                    ...
                                    <?php foreach($imovelTipos as $imoveltipo):?>
                                        <tr>
                                            <th><?=$imoveltipoId()?></th>
                                            <th><?=$imoveltipoNome()?></th>
                                            <td><?=$imoveltipoAtivo()?></td>
                                            <td><?=$imoveltipoCriado()->format('Y-m-d H:i:s')?></td>
                                            <td><?=$imoveltipoModificado()->format('Y-m-d H:i:s')?></td>
                                            <td><?=$imoveltipoCriadorId()?></td>
                                            <td><?=$imoveltipoModificadorId()?></td>
                                            <td>
                                                <ul class="d-flex justify-content-center">
                                                    <li class="mr-3"><a href="/src/View/adminCrud/TiposDeImovel/update.php?id=<?=$imoveltipoId()?>&nome=<?=$imoveltipoNome()?>" class="btn btn-inverse-warning"><i class="bi bi-pencil-square mr-1"></i>Edit</a></li>
                                                    <form method="POST">
                                                        <input type="hidden" name="delete_id" value="<?=$imoveltipoId()?>">
